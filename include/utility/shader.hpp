...
class Shader {
public:
  template <typename... Args> Shader(slist shaders, slist in) {
  ...
  Shader(slist shaders, slist in, slist buf) : Shader(shaders, in) {
    for (auto &b : buf)
      addBuffer(b); // Add Storage Buffers to Shader
  }
  ...
  template <typename T>
  void buffer(std::string name, std::vector<T> &buf, bool update = false);
...
void Shader::buffer(std::string name, std::vector<T> &buf, bool update) {
  ...
  if (update)
    glBufferSubData(GL_SHADER_STORAGE_BUFFER, 0, buf.size() * sizeof(T),
                    &buf[0]);
  else
    glBufferData(GL_SHADER_STORAGE_BUFFER, buf.size() * sizeof(T), &buf[0],
                 GL_STATIC_DRAW);
...
template <typename T> void Shader::uniform(std::string name, T u) {
...
void Shader::uniform(std::string name,
                     const double u) { // GLSL Intrinsically Single Precision
...
template <>
void Shader::uniform(std::string name, const std::vector<glm::mat4> u) {
  glUniformMatrix4fv(glGetUniformLocation(program, name.c_str()), u.size(),
                     GL_FALSE, &u[0][0][0]);
}

template <typename T> void Shader::texture(std::string name, const T &t) {
