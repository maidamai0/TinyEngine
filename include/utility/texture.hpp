...
class Texture;
using tfunc = std::function<void(Texture *)>; // Arbitrary function operating on
                                              // texture pointer

class Texture {
public:
  Texture() { glGenTextures(1, &texture); }; // Default constructor
  Texture(SDL_Surface *s) : Texture() {
    raw(s);
  }; // Load raw surface data into texture
  Texture(int W, int H, bool d = false)
      : Texture() { // Create empty texture of defined size
    if (!d)
      empty(W, H);
    else
      depth(W, H);
  ...
  void
  raw(SDL_Surface *s,
      tfunc param = parameter) { // Generate a texture from raw surface data
    glBindTexture(type, texture);
    glTexEnvi(GL_TEXTURE_ENV, GL_TEXTURE_ENV_MODE, GL_MODULATE);
    glTexImage2D(type, 0, GL_RGBA, s->w, s->h, 0, GL_RGBA, GL_UNSIGNED_BYTE,
                 s->pixels);
...
public:                              // Same thing, 6 times
  ...
  Cubetexture(int W, int H, bool d = false)
      : Cubetexture() { // Create empty texture of defined size
    if (!d)
      empty(W, H);
    else
      depth(W, H);
  ...
  void empty(int W, int H, tfunc param = parameter, GLenum F = GL_RGBA) {
    ...
    for (unsigned int i = 0; i < 6; i++)
      glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X + i, 0, F, W, H, 0, F,
