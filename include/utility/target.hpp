...
public:
  ...
  bool dAttach = false,
       cAttach = true; // Whether we have a depth / color attachment
  GLuint fbo;          // FBO (OpenGL: everything is an int)

  template <typename T> void bind(T &t, bool d) { // Bind a texture to the FBO
    if (d)
      glFramebufferTexture(GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT, t.texture, 0);
    else
      glFramebufferTexture(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, t.texture, 0);
  }

  template <typename T>
  void setup(T &t, T &d) { // Add color and depth
                           // textures!
...
public:
  ...
  Billboard(int W, int H, bool c = true, bool d = true) : Target(W, H, c, d) {
    if (dAttach)
