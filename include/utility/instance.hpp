...
public:
  ...
  void render(
      GLenum mode = GL_TRIANGLE_STRIP); // Default because of primitive models
};

template <typename D> void Instance::addBuffer(std::vector<D> &buf) {
  GLuint instance;
...
template <typename D> void Instance::configBuffer(GLuint instance) {
  glEnableVertexAttribArray(m->vbo.size() + instances.size());
  glVertexAttribPointer(m->vbo.size() + instances.size(),
                        sizeof(D) / sizeof(GLfloat), GL_FLOAT, GL_FALSE, 0,
                        (void *)0);
...
void Instance::configBuffer<glm::mat4>(GLuint instance) {
  for (int i = 0; i < 4; i++) {
    glEnableVertexAttribArray(m->vbo.size() + instances.size());
    glVertexAttribPointer(m->vbo.size() + instances.size(), 4, GL_FLOAT,
                          GL_FALSE, 4 * sizeof(glm::vec4),
...
void Instance::updateBuffer(std::vector<D> &buf, int index) {
  ...
  glBindBuffer(GL_ARRAY_BUFFER,
               instances[index]); // Bind Instance Buffer and Data
  if (buf.size() != SIZE)
    glBufferData(GL_ARRAY_BUFFER, buf.size() * sizeof(D), &buf[0],
                 GL_STATIC_DRAW);
  else
